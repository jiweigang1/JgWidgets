	  ...
	  _initResize:function(){
			...
			self._$resizeMark.on("dragstart",function(e){
				...
				var  t = self._$resizeMark.clone().addClass("protx").appendTo(self._$box);	 
					...
					if(self._$resizeMark.data("fth")){
						 t.data("fth"	  ,self._$resizeMark.data("fth"));
					} 
			...
			self._$resizeMark.on("dragend",function(e,t){
				...
				if($(t.proxy).data("fth")){
					self._addThFixWidth($(t.proxy).data("fth"),t.deltaX );
				}
				$(t.proxy).remove();
			});
			
		this._$theadTable.find("tr:not(.cindex)>th").each(function(){
			var $this = $(this);
			if($this.data(self._th_data_name).fix){
				return true;
			}
