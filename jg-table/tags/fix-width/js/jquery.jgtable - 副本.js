    ...
    $.widget("jgWidgets.jgTable", {
	 ...
	 _initt:function(){
	  ...
	  _initHeadHtml:function(){
			...
			var ths ='<colgroup style="height:auto" class="cindex" >';
			for(var i=0;i<this._headerData[0].length;i++){
				ths+='<col></col>';
			}
				ths+="</colgroup>";
			var $ths = $(ths).insertBefore(this._$theadTable.find("thead")).find("col");
	  ...
	  _initBodyHtml:function(){
		...
		this._$tbodyTable.prepend(this._$theadTable.find(".cindex").clone());
		
		var $ths = this._$tbodyTable.find(".cindex>col");
		for(var i=0;i<this._headerData.length;i++){
			for(var k=0;k<this._headerData[i].length;k++){
	  ...
	  _initFootHtml:function(){
		if(this._$tbodyTable.find("tfoot").length>0){
			...
			this._$tfootTable.width(this._settings._width).prepend(this._$theadTable.find(".cindex").clone());
			
			var $ths = this._$tfootTable.find(".cindex>col");
			for(var i=0;i<this._headerData.length;i++){
			...
			if(this._settings.cloneHeadToFoot){
				this._$tfootTableContainer = $('<div  class="jg-table-tfoot-container" ><table class="jg-table-tfoot jg-table" ></table></div>').appendTo(this._$box);
				this._$tfootTable = $(".jg-table-tfoot",this._$tfootTableContainer);
				this._$tfootTable.append('<thead></thead>').width(this._settings._width).prepend(this._$theadTable.find(".cindex").clone());
				var $ths = this._$tfootTable.find(".cindex>col");
				var hd	=[];
				var html ="";
				var $p = $("<p></p>");
	  ...
	  _initResize:function(){
			...
			self._$resizeMark.draggable({
				...
				stop:function(e,ui){
					...
					self._$tbodyTableContainer.trigger("mousemove");
					self._updateScroll();
				}
			});
