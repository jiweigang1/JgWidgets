...
(function ($) {
    $.widget("jgWidgets.jgTable", {
	 ...
	 _initt:function(){
	  ...
	  _createRowsByJson:function(data){
	  ...
	  _loadData:function(url,params,type,append){
		var self = this;
		$.ajax({
			...
			if(type==="json"){
				html = self._createRowsByJson(html);
			}
			var $body = self._$tbodyTable.find("tbody");
			if(!append){
				$body.empty();
			}
			$body.append(html);
			self._resetFixedTableData();
			self._updateScroll();
		}).fail(function(){
			
		});
	  },
	  loadData:function(url,params,type,append){
		if(arguments.length==1&&typeof arguments[0]==="object"){
			params = arguments[0].params;
			type   = arguments[0].type;
			append = arguments[0].append;
			url    = arguments[0].url;
		}
		var self = this;
		if(!type){
		...
		self._loadData(url,params||{},type,append);
	  },
